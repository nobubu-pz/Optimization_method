...
def HyperPara(x, y, theta, kernel_type = "GP"):
    if kernel_type == "GP":
        N = len(x)
        # M = len(x_)
        K_theta = [[Gauss_kernel.Gauss_kernel(x[i][:], x[j][:], i, j, theta) for j in range(N)] for i in range(N)]
        ...
        dK_dtheta = Gauss_kernel.Gradient_K(x, theta)
